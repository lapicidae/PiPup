...
class SettingsActivity : AppCompatActivity() {
    ...
    private fun setupSubmenuFocus(navId: Int) {
        ...
        val focusableChildren = (0 until container.childCount).map { container.getChildAt(it) }.filter { it.isFocusable && it.isVisible }
    ...
    private fun updatePreview(animate: Boolean = false) {
        ...
        handler.postDelayed({
            ...
            if (existingPreview != null && !animate) {
                ...
                getController(currentLayoutRes).updatePreviewPosition(currentFocus ?: existingPreview)
            } else {
                ...
                binding.previewArea.addView(preview, FrameLayout.LayoutParams(-2, -2).apply {
    ...
    class ColorSpinnerAdapter(context: Context, val colors: List<ColorEntry>, val defaultHex: String) : ArrayAdapter<ColorEntry>(context, 0, colors) {
        ...
        private fun create(p: Int, v: View?, g: ViewGroup): View {
            ...
            label.text = if (entry.hex.equals(defaultHex, true)) "$name${context.getString(R.string.settings_default_suffix)}" else name
